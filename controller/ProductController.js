...
class Product {
  ...
  async getSearchSort(req, res) {
    try {
      ...
      if (
        sortparam !== undefined &&
        !["price", "rating", "discountPercentage", "stock"].includes(sortparam)
      ) {
      ...
      if (
        ...
        errors.push(
          "Invalid parameter for sortorder has been provided or cannot be blank."
        );
      }
      ...
      if (errors.length > 0) {
        return res
          .status(400)
          .send(failure("Invalid query parameters", errors));
      }
      ...
      return res
        .status(400)
        .send(failure("No products were found according to your search!!!"));
    } catch (error) {
      console.log(error);
      return res.status(400).send(failure("Internal server error"));
    }
  ...
  async create(req, res) {
    try {
      ...
      return res.status(400).send(failure("Failed to add the Product"));
    } catch (error) {
      console.log(error);
      return res.status(400).send(failure("Internal server error"));
    }
  ...
  async getProductByID(req, res) {
    try {
      ...
      if (product) {
        return res
          .status(200)
          .send(success("Great! Here is the searched Product.", product));
      } else {
